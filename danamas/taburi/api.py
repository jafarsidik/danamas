...
from danamas.taburi.doctype.whatsapp.whatsapp import incoming_message_callback
...
def incoming_whatsapp_message_handler(**kwargs):
	...
	# Add a message
	resp.message(frappe.db.get_single_value('Whatsapp Setting', 'reply_message'))
	return Response(resp.to_xml(), mimetype='text/xml')
...
def whatsapp_message_status_callback(**kwargs):
	...
	"""
	args = frappe._dict(kwargs)
	if frappe.db.exists({'doctype': 'WhatsApp', 'id': args.MessageSid, 'from': args.From, 'to': args.To}):
		message = frappe.get_doc('WhatsApp Message', {'id': args.MessageSid, 'from': args.From, 'to': args.To})
		message.db_set('status', args.MessageStatus.title())
@frappe.whitelist(allow_guest=True)
def testing_api(**kwargs):
	# Find your Account SID and Auth Token at twilio.com/console
	# and set the environment variables. See http://twil.io/secure
	account_sid = frappe.db.get_single_value('Whatsapp Setting', 'account_sid')
	auth_token  = frappe.db.get_single_value('Whatsapp Setting', 'auth_token')
