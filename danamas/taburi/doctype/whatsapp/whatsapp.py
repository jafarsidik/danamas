...
class WhatsApp(Document):
	...
	def get_message_dict(self):
		args = {
			...
			'to': self.to,
			'body': self.message,
			'status_callback': '{}/api/method/danamas.taburi.api.whatsapp_message_status_callback'.format(get_site_url(frappe.local.site))
		}
		if self.media_link:
	...
	def store_whatsapp_message(to, message, doctype=None, docname=None, media=None):
		sender = frappe.db.get_single_value('Whatsapp Setting', 'sender_number')
		wa_msg = frappe.get_doc({
				'doctype': 'WhatsApp',
				'from': 'whatsapp:{}'.format(sender),
